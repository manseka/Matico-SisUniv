...
@section('content_header')
    <h1>Listado Personal Docente</h1>
    <br>
@stop
    ...
    <div class="col-md-12"> <!-- INICIO columna central -->
        <div class="card card-outline card-primary"> <!-- INICIO card -->
            ...
            <div class="card-header d-flex justify-content-between align-items-center"> <!-- INICIO card header -->
                ...
                <a href="{{ url('admin/docentes/create') }}" class="btn btn-primary btn-sm ml-auto">
                    ...
                    <tbody> <!-- INICIO cuerpo tabla -->
                        ...
                        @foreach ($docentes as $docente)
                            <tr>
                                ...
                                <td class="text-center">{{ $docente->usuario->roles->pluck('name')->implode(', ') }}</td>
                                <td class="text-center">{{ $docente->nombres }}</td>
                                <td class="text-center">{{ $docente->apellidos }}</td>
                                <td class="text-center">{{ $docente->dni }}</td>
                                <td class="text-center">{{ $docente->telefono }}</td>
                                <td class="text-center">{{ $docente->direccion }}</td>
                                <td class="text-center">{{ $docente->usuario->email }}</td>
                                <td class="text-center">{{ $docente->profesion }}</td>

                                <td class="text-center">
                                    @if ($docente->foto)
                                    <img class="thumb thumbnail" src="{{ asset('storage/docentes/' . $docente->foto) }}" alt="Foto" width="70%" title="Foto">
                                    </img>
                                ...
                                <td class="text-center">
                                    <a href="{{ url('admin/docentes/' . $docente->id ) }}" class="btn btn-info btn-sm">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ url('admin/docentes/' . $docente->id . '/edit') }}" class="btn btn-warning btn-sm">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" class="btn btn-danger btn-sm" onclick="confirmarEliminacion({{ $docente->id }})">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                    <form id="form-eliminar-{{ $docente->id }}" action="{{ url('admin/docentes/' . $docente->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
